...
function Ide3(window, document, translate, gUserId, pagePanic) {
  ...
  function Storage() {
    ...
    this.saveChanges = wrapException(saveChanges, "saveChanges");
  }
  ...
  function addCentral(node, centralMachine) {
    createCentralCore(node, centralMachine);
  }
  ...
  function addClickable(popup, text, action, style) {
    ...
    HtmlUtils.setDivText(row, text);
    row.onclick = wrapException(action, "searchGroup");
  }
  ...
  function addError(popup, item) {
    var row = make(popup, "div");
    row.className = "list_item";
    row.style.borderBottom = "solid 1px silver";
    ...
    HtmlUtils.setDivText(contentDiv, translate(item.message));
  }
  ...
  function addExpandedGroup(items) {
    ...
    while (true) {
      ...
      if (_sw33580000_ === "folder") {
        addSearchFolder(list, item);
      } else {
        if (_sw33580000_ === "item") {
          addSearchItem(list, item);
        }
      }
      _ind3347++;
    }
    ...
    if (items.length == 0) {
      ...
      while (true) {
        ...
        if (_sw32810000_ === "folder") {
          addSearchFolder(gl.div, item);
        } else {
          if (_sw32810000_ === "item") {
            addSearchItem(gl.div, item);
          }
        }
        _ind3260++;
      }
  ...
  function addIconText(list, image, text) {
    var row = {
      id: text,
      cells: [{ src: image, width: 36, height: 36 }, { text: text }],
    };
    list.push(row);
  }
  ...
  function addModuleRow(table, item) {
    ...
    var td = createIconListRow(table, "module.png", action);
  ...
  function addPopupItem(div, item) {
    if (item.type == "separator") {
      ...
      HtmlUtils.setDivText(textPart, translate(item.text));
      var itemId = item.id || "";
  ...
  function addProjectRow(table, project) {
    ...
    var td = createIconListRow(table, "workspace-s2.png", action);
    td.style.padding = "5px";
  ...
  function addRecentRow(table, item) {
    ...
    var td = createIconListRow(table, item.icon, action);
    td.style.padding = "5px";
  ...
  function addSearchFolder(popup, item) {
    ...
    table.style.borderBottom = "solid 1px silver";
  ...
  function addSearchItem(popup, item) {
    ...
    var before = found.line.substring(0, found.start);
    var selected = found.line.substring(
      found.start,
      found.start + found.length,
    );
    var after = found.line.substring(found.start + found.length);
      ...
      if (globs.search.expanded) {
        ...
        while (true) {
          ...
          if (items && items.length > 0) {
            addFoundGroup(group, defs.completed, items);
            changed = true;
          }
          _ind3505++;
        }
        ...
        if (defs.completed) {
          removeEmpty();
          if (total == 0) {
            addEmpty(translate("MES_NOTHING_FOUND"));
          }
        } else {
          if (changed) {
            removeEmpty();
        ...
        var error = {
          ...
          errors: [
            ...
            },
            {
              ...
              path: "Foo/Bar/Moo",
              name: "d03",
              message: "Life is getting better",
  ...
  function buildDashboardWidget(div, node, widget) {
    ...
    var bStyle = "solid 1px " + DarkBackground;
    if (globs.wide) {
    ...
    createButton(myPane, "MES_MY_PROJECT", goToMyProject, SpecialBack);
    createButton(myPane, "MES_SPACES", goToSpaces, NormalBack);
    createLink(myPane, "MES_ACCOUNT", "/account");
    createLink(myPane, "MES_HOMEPAGE", Root);
    createLink(myPane, "MES_TUTORIALS", "https://drakon.tech/read/tutorials");
    var host = "https://drakonhub.com";
    docs.style.borderTop = bStyle;
    createHeader(docs, "MES_MODULES");
    widget.modules = make(docs, "div");
    applyDashStyle(recentPane);
    createHeader(recentPane, "MES_RECENT");
    widget.recent = make(recentPane, "div");
    applyDashStyle(projectsPane);
    createHeader(projectsPane, "MES_SPACES");
    widget.projects = make(projectsPane, "div");
  }
  ...
  function buildLogonGui() {
    ...
    return {
      ...
      kids: [title, username, password, forgot, status, ok, cancel],
    };
  }
  ...
  function buildNormalCheckbox(parent, node) {
    ...
    if (node.onchange) {
      checkbox.onchange = wrapException(node.onchange, "normalCheckbox");
    }
  }
  ...
  function buildSignupGui() {
    ...
    if (Config.EMAIL_SIGNUP) {
      kids = [title, userLab, username, chars3, emailLab, email, status];
    } else {
      kids = [
        title,
        userLab,
  ...
  function buildUrlForFolder(spaceId, folderId) {
    ...
    return "https://" + host + "/ide/doc/" + spaceId + "/" + folderId;
  }

  function canBuildNow() {
    var current = self.logic.getCurrent();
    if (
      (current.screen == "middle_diagram" ||
        current.screen == "middle_folder") &&
      !current.isReadonly &&
      !HtmlUtils.hasPopup() &&
      !isCentralVisible()
    ) {
  ...
  function createBuildWindow() {
    ...
    var winTitle = InputBox.makeWindowTitle(root, translate("MES_CODEGEN"));
  ...
  function createButton(parent, textId, action, color) {
    ...
    div.onclick = wrapException(action, "buttonClick");
  ...
  function createCentralBack() {
    ...
    style.opacity = 0.3;
    back.onclick = wrapException(killCentral, "killCentral");
    back.oncontextmenu = function () {
      return false;
    };
  }
  ...
  function createCentralCore(node, centralMachine) {
    ...
    var id = globs.widgets.createWidget(central, node);
  ...
  function createEditor() {
    ...
    var inputBox = {
      show: function (title, text, onSave, validate, x, y, cmo) {
        showInputBox(false, title, text, onSave, validate, false, x, y, cmo);
      },
    };
    var cfg = {
      imageBase: ImagePath,
      ...
      panic: panic,
      saveUserSettings: wrapException(saveUserSettings, "saveUserSettings"),
  ...
  function createFeedbackButton() {
    if (globs.wide && !globs.tryMe) {
      ...
      button.id = "feedback";
      HtmlUtils.setDivText(button, translate("MES_SEND_FEEDBACK"));
      setButtonStyle(button, "black", SpecialBack, "3px");
  ...
  function createIconListRow(table, icon, action) {
    ...
    img.src = path;
    if (action) {
      tr.onclick = wrapException(action, "listClick");
    }
    return td;
  }
  ...
  function createSearchList(defs, found) {
    globs.search = {};
    var search = findSearch();
    globs.search.defs = Utils.copyObjectDeep(defs);
  ...
  function createSearchPopup() {
    ...
    var searchWidget = getWidget(globs.search.widgetId);
    var tellLogic = function () {
      self.logic.resetSearch();
    };
    popup = HtmlUtils.createSoftPopup(searchWidget.rect, tellLogic);
    popup.style.background = "white";
    popup.style.maxWidth = width + "px";
    popup.style.width = width + "px";
    popup.className = "popup";
    var top = searchWidget.rect.top + searchWidget.rect.height;
    var start = top + 50;
    ...
    addClickable(popup, translate("MES_CLOSE"), cancelSearch, closeStyle);
    var popupList = make(popup, "div");
    popupList.style.maxHeight = "calc(100vh - " + start + "px)";
    popupList.style.overflowY = "auto";
    popup.style.display = "inline-block";
    popup.style.position = "fixed";
    globs.search.popup = popupList;
    addEmpty(translate("MES_SEARCHING"));
  }
  ...
  function createUserList(id, userSet) {
    ...
    while (true) {
      ...
      addIconText(node.rows, "delete.png", user);
      _ind1873++;
    }
    return node;
  }
  ...
  function destroyBuildWindow() {
    if (globs.build) {
      document.body.removeChild(globs.build.root);
  ...
  function doSendFeedback(text) {
    ...
    showWorking();
    sendFeedbackCore(data, success, error);
  }
  ...
  function findSearch() {
    ...
    if (topPanel) {
      ...
      return findWidgetInTree(topPanel, isSearch);
    } else {
      return null;
    }
  }
  ...
  function getDateString() {
    ...
    return pad1(yyyy) + pad1(mm) + pad1(dd) + pad1(hh) + pad1(m) + pad1(s);
  }
  ...
  function getOrCreateExpanded(group) {
    if (globs.search.expandedList) {
      ...
      addClickable(popup, caption, toAllResults);
  ...
  function hideDemo() {
    ...
    if (check) {
      if (check.checked) {
        HtmlUtils.deleteCookie("demo-discarded");
      } else {
        HtmlUtils.setCookie("demo-discarded", "discarded", 20);
      }
      destroyDemoWindow();
    }
  }
  ...
  function initControls(wide, isTryMe) {
    ...
    var rootDiv = get("wide");
    globs.root = globs.widgets.createWidget(rootDiv, rootNode);
  ...
  function isDemoDiscarded() {
    var discarded = HtmlUtils.getCookie("demo-discarded");
    return !!discarded;
  }
  ...
  function loadFile() {
    ...
    if (path) {
      ...
      HtmlUtils.setText("file-error", translate("MES_NO_FILE_CHOSEN"));
    }
  }
  ...
  function loadFontCore(font, target) {
    ...
    if (file in globs.loadedFonts) {
      console.log("Font already loaded ", file);
      scheduleNextState(target, null);
    } else {
      ...
      addStyle(ruleText);
      var fontSpec = makeFontSpec2(font.italic, font.bold);
      var font2 = new FontFaceObserver(font.family, fontSpec);
      var timeout = Config.FONT_TIMEOUT * 1000;
      font2.load(null, timeout).then(onSuccess, onError);
    }
  }
  ...
  function makeBigPictureButton(div, node, widget) {
    ...
    registerEvent(div, widget, "click");
  }
  ...
  function makeRecoverLink(div) {
    var a = make(div, "a");
    a.href = "/reset";
    HtmlUtils.setDivText(a, translate("MES_FORGOT_PASSWORD"));
  }
  ...
  function makeSearchCallback(item) {
    ...
    var action = function () {
      ...
      hideHelp();
      hideDemo();
      self.logic.onSearchItem(type, target);
    };
    return wrapException(action, "searchItem:" + type + ":" + target);
  }
  ...
  function makeTopWidgetsDesc() {
    ...
    var folderTop = {
      ...
      type: "hdock",
      height: TopPanelHeight,
      lefts: [mainMenu, back, forward, up, create0, sSpacer, build],
      rights: [search],
      style: { background: DarkBackground },
      center: {
        type: "path",
    ...
    var diagramTopTry = {
      ...
      type: "hdock",
      height: TopPanelHeight,
      lefts: [mainMenu, saveButton, undoButton, redoButton, exp4],
      rights: [],
      style: { background: DarkBackground },
      center: {
        type: "path",
  ...
  function makeTopWidgetsDescMob() {
    ...
    var diagramTop = {
      type: "hdock",
      height: TopPanelHeight,
      lefts: [mainMenu, create, undoButton, redoButton, build],
      rights: rights,
      style: { background: DarkBackground },
    };
    var diagramHead = {
  ...
  function onDefResult(cx, cy, queryId, current, data, calls) {
    ...
    if (extended && queryId == extended.queryId) {
      ...
      if (data.folders && data.folders.length > 0) {
        ...
        while (true) {
          ...
          if (
            item.space_id == current.spaceId &&
            !(item.folder_id == current.folderId)
          ) {
        ...
        if (calls.length > 0 && !isReadonly()) {
          var call = calls[0];
          var text = translate("MES_CREATE") + " " + call.name;
          var action = function () {
            createFunctionStub(call);
          };
          menuItems.push({
            type: "item",
            text: text,
      ...
      if (menuItems.length == 0) {
        ...
        ContextMenu.repos(cx, cy);
      }
    }
  }

  function createFunctionStub(call) {
    self.logic.createFunctionStub(call);
  }
  ...
  function onError(errorMsg, url, lineNumber, column, errorObj) {
    reportError(errorMsg, url, lineNumber, column, errorObj);
    pagePanic(errorMsg);
  }
  ...
  function onEvent(evt, type, widget, rowId, cellId) {
    ...
    if (myHandler) {
      myHandler(evt, type, widget, rowId, cellId);
    } else {
      self.logic.onEvent(evt, type, widget, rowId, cellId);
    }
  }
  ...
  function onFontLoaded(file, target, font) {
    ...
    var face = Utils.buildFontFace(font.italic, font.bold, font.family);
    console.log("Font ready", file, face);
  ...
  function onFontNotLoaded(file, target) {
    console.log("Font NOT loaded", file);
  ...
  function onResize() {
    ...
    globs.widgets.resizeWidget(globs.root, left, top, width, height);
    resizeCentral();
    //resizeCentral()
  }
  ...
  function orderResize() {
    ...
    var timer = setTimeout(callback, ResizeTimeout, "orderResize");
  ...
  function panic(data) {
    ...
    if (shouldReport(rawMessage)) {
      reportError(message, url, lineNumber, column, errorObj);
      pagePanic(message);
    } else {
      if (rawMessage == "ERR_NOT_LOGGED_ON") {
        var url = "/logon?target=" + getMyUrl();
        goToUrl(url);
      } else {
        pagePanic(message);
      }
  ...
  function positionSearchResults() {
    ...
    HtmlUtils.showUnder(widget, popup);
  }
  ...
  function promiseError(ex) {
    onError(ex.message, ex.fileName, ex.lineNumber, null, ex);
  }
  ...
  function registerShortcuts() {
    ...
    bindEditor("up", function () {
      editor.arrowUp(false, false, false);
    });
    bindEditor("mod+up", function () {
      editor.arrowUp(true, false, false);
    });
    bindEditor("shift+up", function () {
      editor.arrowUp(false, true, false);
    });
    bindEditor("alt+up", function () {
      editor.arrowUp(false, false, true);
    });
    bindEditor("down", function () {
      editor.arrowDown(false, false, false);
    });
    bindEditor("mod+down", function () {
      editor.arrowDown(true, false, false);
    });
    bindEditor("shift+down", function () {
      editor.arrowDown(false, true, false);
    });
    bindEditor("alt+down", function () {
      editor.arrowDown(false, false, true);
    });
    bindEditor("left", function () {
      editor.arrowLeft(false, false, false);
    });
    bindEditor("mod+left", function () {
      editor.arrowLeft(true, false, false);
    });
    bindEditor("shift+left", function () {
      editor.arrowLeft(false, true, false);
    });
    bindEditor("alt+left", function () {
      editor.arrowLeft(false, false, true);
    });
    bindEditor("right", function () {
      editor.arrowRight(false, false, false);
    });
    bindEditor("mod+right", function () {
      editor.arrowRight(true, false, false);
    });
    bindEditor("shift+right", function () {
      editor.arrowRight(false, true, false);
    });
    bindEditor("alt+right", function () {
      editor.arrowRight(false, false, true);
    });
  ...
  function resizeCentral() {
    ...
    if (id) {
      ...
      globs.widgets.resizeWidget(id, x, y, width, height);
    }
  }
  ...
  async function saveAsPng(zoom) {
    var exportCanvas = make(document.body, "canvas");
  ...
  function saveDiaProps() {
    ...
    keywords["algoprop"] =
      isChecked("function_flags_algoprop") ||
      isChecked("function_flags_lazy_algoprop");
  ...
  function schedule(target, ms) {
    ...
    return setTimeout(callback, ms, "schedule");
  }
  ...
  function selectDemoButton(id) {
    ...
    while (true) {
      ...
      var buttonId = _keys4167[_ind4167];
      var rec = _col4167[buttonId];
  ...
  function sendDelete(url, target) {
    ...
    HtmlUtils.sendRequest("DELETE", url, {}, success, error);
  }
  ...
  function sendFeedback() {
    ...
    showInputBox(true, translate(title), "", onSave, enforceNotEmpty, false);
  }
  ...
  function sendGet(url, target) {
    ...
    HtmlUtils.sendGet(url, success, error);
  }
  ...
  function sendPost(url, payload, target) {
    ...
    HtmlUtils.sendPost(url, payload, success, error);
  }
  ...
  function sendPut(url, payload, target) {
    ...
    HtmlUtils.sendRequest("PUT", url, payload, success, error);
  }
  ...
  function setAccessError(message) {
    ...
    HtmlUtils.setDivText(label, message);
    resizeCentral();
  }
  ...
  function setFoundUsers(users) {
    ...
    while (true) {
      ...
      addIconText(rows, "user-s.png", user);
      _ind2742++;
    }
  ...
  function setMobileInputError(message) {
    HtmlUtils.setText("mobile_input_error", message);
    resizeCentral();
  }
  ...
  function shouldReport(message) {
    if (
      ...
      message == "ERR_COMMUNICATION" ||
      message == "ERR_DIAGRAM_LIMIT" ||
      message == "ERR_USER_LIMIT" ||
      message == "ERR_SPACE_LIMIT" ||
      message == "ERR_NOT_LOGGED_ON"
    ) {
      return false;
    } else {
      return true;
  ...
  function showAccessScreen(access, machine) {
    ...
    var bottomPage = {
      type: "page",
      padding: 10,
      kids: [readHead, read, writeHead, write, adminHead, admin],
    };
    var root = {
      type: "vdock",
      top: topPage,
  ...
  async function buildPseudocodeWindow(container, pseudo) {
    ...
    var copyButton = createButton(top, "MES_COPY", async function () {
      await navigator.clipboard.writeText(pseudo);
      copyButton.remove();
    });
  ...
  function showBuild(options) {
    ...
    HtmlUtils.setDivText(module, options.module);
    ...
    if (_sw46310000_ === "success") {
      ...
      success.innerHTML = "&#10004; " + translate("MES_SUCCESS");
      if (options.url) {
      ...
      HtmlUtils.setDivText(close, translate("MES_CLOSE"));
      ...
      if (_sw46310000_ === "error") {
        ...
        success.style.padding = "10px";
        success.style.margin = "5px";
        success.innerHTML = "&#10060; " + translate("MES_ERROR");
        var generate = make(client, "div");
        generate.className = "panic_button";
        generate.style.margin = "5px";
        HtmlUtils.setDivText(generate, translate("MES_GENERATE"));
        var close = make(client, "div");
        close.className = "panic_button";
        close.style.margin = "5px";
        HtmlUtils.setDivText(close, translate("MES_CLOSE"));
        ...
        success.style.padding = "10px";
        success.style.margin = "5px";
        success.innerHTML = translate("MES_WORKING");
        var generate = make(client, "div");
        generate.className = "panic_button";
        ...
        HtmlUtils.setDivText(cancel, translate("MES_CANCEL"));
  ...
  function showChangeDiaProps(machine, name, folder, ro) {
    globs.keywords = Utils.copyObject(folder.keywords || {});
    ...
    var funType = {
      ...
      function:
        globs.keywords["function"] ||
        (!globs.keywords["algoprop"] && !globs.keywords["machine"]),
  ...
  function showChangeFolderProps(
    machine,
    name,
    props,
    saveLabel,
    ...
    var normal = {
      type: "vpanel",
      kids: [roof, space, header, space, footer, space, basement],
    };
    var many = {
      type: "custom",
  ...
  function showContextMenu(x, y, menu, onBackground, item) {
    ...
    while (true) {
      ...
      var wrapped = wrapException(it.code, "context-menu:" + itemId);
      it.code = wrapped;
      _ind1745++;
    }
    ContextMenu.show(x, y, menu, onBackground);
    var extended = {};
    ...
    if (item && !globs.tryMe && canDefQueryFrom(item)) {
      ...
      var onSuccess = function (data) {
        onDefResult(x, y, queryId, current, data, calls);
      };
      var body = {
        space_id: spaceId,
        lines: item.lines,
  ...
  function tryGetFunctionNames(item) {
    ...
    var wrapped = "async function _wrp_() { " + item.text + "\n}";
    var call;
    try {
      var parsed = window.esprima.parseScript(wrapped);
      return tryExtractCall(parsed);
    } catch (ex) {
      return [];
    }
  }

  function tryExtractCall(parsed) {
    var expr = parsed.body[0].body.body;
    var calls = [];
    extractCallRecursive(expr, calls);
    return calls.map(normalizeCall).filter((call) => Boolean(call));
  }

  function normalizeCall(callExpr) {
    ...
    for (var i = 0; i < callExpr.arguments.length; i++) {
      ...
      if (arg.type === "Identifier") {
        call.args.push(arg.name);
      } else {
        call.args.push("value" + (i + 1));
      }
    }
    return call;
  }
  ...
  function extractCallRecursive(node, output) {
    ...
    if (Array.isArray(node)) {
      ...
      for (var key in node) {
        var value = node[key];
        extractCallRecursive(value, output);
      }
    }
  }
  ...
  function showDemo(type) {
    ...
    var width = videoMargin * 2 + panelWidth + videoWidth;
    ...
    back.onclick = wrapException(hideDemo, "hideDemo");
    showFullscreen(back);
    ...
    var id = globs.widgets.createWidget(dialog, node);
    globs.demo.widgetId = id;
    globs.widgets.resizeWidget(id, 0, 0, width, height);
  ...
  function showInputBox(
    ...
    InputBox.show(isAsync, header, old, onSave, validate, x, y, cmOptions);
  }
  ...
  function showShareScreen(
    ...
    var url = buildUrlForFolder(spaceId, folderId);
    var title;
    if (type == "folder") {
  ...
  function showUrlScreen(module, urls, machine) {
    ...
    var kids = [moduleLabel, titleLabel, edit, cancel];
    var root = {
      type: "page",
      style: {
        background: "white",
    ...
    if (groupId) {
      ...
      while (true) {
        ...
        if (group.items.length > 0) {
          addFoundGroup(group, defs.completed, group.items);
        }
        _ind4348++;
      }
    }
  ...
  function updateFormatList() {
    ...
    if (_sw51980000_ === "LANG_JS") {
      ...
      if (_sw51980000_ === "LANG_JS2") {
        ...
        allowed = ["MES_MODULE", "MES_PROGRAM"];
        def = "MES_PROGRAM";
      } else {
        addOption(select, "MES_NONE", translate("MES_NONE"));
        allowed = ["MES_NONE"];
